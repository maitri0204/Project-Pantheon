...
export const emailStudentAttemptReport = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const shouldGenerateOnServer = hasClientPdf
    ? false
    : Boolean(canGenerateOnServer && (serverGenerate || !pdfBase64));

  if (!shouldGenerateOnServer && !hasClientPdf) {
    res.status(400).json({
      message: requiresClientPdf
        ? "The full report PDF could not be received. Please wait for report generation to finish and try again."
        : canGenerateOnServer
          ? "A report PDF upload is required"
          : "Email report is not available for this assessment.",
    });
    return;
  }
