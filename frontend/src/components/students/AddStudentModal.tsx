...
const GRADE_OPTIONS = ["6", "7", "8", "9", "10", "11", "12", "Other"];
...
export default function AddStudentModal({ open, token, onClose, onSuccess }: AddStudentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.grade === OTHER_GRADE_VALUE && !resolvedGrade) {
        setError("Please enter grade / level.");
        return;
      }

      await apiRequest("/superadmin/students", {
        method: "POST",
        body: JSON.stringify({
          ...form,
      ...
      <div className="w-full max-w-4xl rounded-2xl bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Student will be added under Kareer Studio by default.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <FieldWrap label="Grade / Level" required={form.grade === OTHER_GRADE_VALUE}>
              <select value={form.grade} onChange={(e) => { setField("grade", e.target.value); if (e.target.value !== OTHER_GRADE_VALUE) setOtherGrade(""); }} className={selectCls} required={form.grade !== OTHER_GRADE_VALUE}>
                <option value="">Select grade / level</option>
                {GRADE_OPTIONS.map((g) => <option key={g} value={g}>{g}</option>)}
              </select>
              {form.grade === OTHER_GRADE_VALUE && (
                <input type="text" value={otherGrade} onChange={(e) => setOtherGrade(e.target.value)} className={`${inputCls} mt-2`} placeholder="e.g. B.Tech, B.Com 1st year" required />
              )}
            </FieldWrap>
            <FieldWrap label="Division">
              <input type="text" value={form.division} onChange={(e) => setField("division", e.target.value.toUpperCase().slice(0, 3))} className={inputCls} placeholder="A" />
            </FieldWrap>
          </div>
