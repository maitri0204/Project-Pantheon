...
import {
  LANDING_ASSESSMENT_CATALOG,
  LANDING_LITMUS_ASSESSMENT,
  type LandingAssessmentDetail,
} from "@/lib/landingAssessmentCatalog";
...
function AssessmentCard({
  ...
  test: LandingAssessmentDetail;
  className?: string;
}) {
  return (
    <article className={`test-card p-6 sm:p-8 ${className}`}>
      ...
      <div className="relative space-y-5 pt-2">
        ...
        <div>
          <p className="mb-2.5 text-sm font-bold uppercase tracking-wide text-slate-500">What it measures</p>
    ...
    >
      <div className="mx-auto mb-12 max-w-3xl space-y-4 text-center">
        <h2 className="text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">
          Explore Our <span className="gradient-title">9 Assessments</span>
        </h2>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">
        {LANDING_ASSESSMENT_CATALOG.map((test) => (
          <AssessmentCard key={test.name} test={test} />
        ))}
        <AssessmentCard test={LANDING_LITMUS_ASSESSMENT} className="md:col-span-2" />
      </div>
    </section>
  );
}
