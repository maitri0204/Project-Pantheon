      ...
      <div className="rounded-3xl border border-blue-200/70 bg-gradient-to-br from-blue-600 via-cyan-600 to-indigo-700 p-6 text-white shadow-[0_28px_70px_-30px_rgba(37,99,235,0.75)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black tracking-tight">{parent.firstName} {parent.lastName}</h1>
            <p className="mt-2 flex items-center gap-2 text-sm text-blue-100"><Mail className="h-4 w-4" /> {parent.email}</p>
            <p className="mt-1 flex items-center gap-2 text-sm text-blue-100"><Phone className="h-4 w-4" /> {`${parent.phoneCode || ""}${parent.phone || ""}`.trim() || "—"}</p>
          </div>
