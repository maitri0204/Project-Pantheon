...
export default function ReviewerPaymentFlow() {
  ...
  const startPayment = async () => {
    const auth = getStoredAuth();
    if (!auth) return window.alert("Not authorized");
  ...
  if (!visible) return (
    <div className="rounded-3xl border border-blue-200 bg-white/90 p-5 shadow-lg">
      <p className="text-sm text-slate-700">Reviewer access required to make a payment.</p>
    </div>
  );
  ...
  return (
    <div className="rounded-3xl border border-blue-200 bg-white/90 p-5 shadow-lg">
      ...
      <h3 className="mt-2 text-2xl font-black text-slate-950">Reviewer Payment</h3>
      <p className="mt-2 text-sm leading-6 text-slate-600">Click below to open Razorpay and complete the transaction.</p>

      <button
        onClick={startPayment}
        disabled={loading}
        className="mt-5 inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition hover:from-blue-700 hover:to-cyan-700 disabled:opacity-60"
      >
