...
export default function QuestionsPage() {
  ...
  // Category filter within an assessment
  ...
  const [editForm, setEditForm] = useState({ title: "", questionText: "" });
  ...
  useEffect(() => {
    ...
    apiRequest<SuperadminResponse>("/superadmin/dashboard", {}, auth.token)
      .then((res) => {
        ...
        if (res.assessments.length > 0) setActiveCode(res.assessments[0].code);
      })
  ...
  // Load questions when active assessment changes
  const loadQuestions = useCallback(async (code: string) => {
    ...
    try {
      const res = await apiRequest<QuestionsResponse>(
        `/superadmin/assessments/${code}/questions`, {}, auth.token
      );
      if (res.questions.length === 0 && code === "METACOGNITION_TEST") {
        const legacy = await apiRequest<QuestionsResponse>(
          "/superadmin/assessments/METACOGNITION/questions", {}, auth.token
        );
        setQuestions(legacy.questions.map((q) => ({ ...q, assessmentCode: "METACOGNITION_TEST" })));
      } else {
        setQuestions(res.questions.map((q) => ({ ...q, assessmentCode: normalizeAssessmentCode(q.assessmentCode) })));
      }
    } catch {
      setQuestions([]);
    } finally {
  ...
  const scopedQuestions = useMemo(
    () => questions.filter((q) => normalizeAssessmentCode(q.assessmentCode) === activeCode),
  ...
  const categories = useMemo(() => {
    ...
    scopedQuestions.forEach((q) => seen.set(q.category, q.categoryLabel));
    return Array.from(seen.entries()).map(([cat, label]) => ({ cat, label }));
  }, [scopedQuestions]);

  const filteredQuestions = useMemo(() => {
    return scopedQuestions.filter((q) => activeCat === "ALL" || q.category === activeCat);
  }, [scopedQuestions, activeCat]);

  // Add form default next question number
  useEffect(() => {
    if (showAdd) {
      ...
      setAddForm((f) => ({
        ...
        category: selectedCategory?.cat || f.category,
        categoryLabel: selectedCategory?.label || f.categoryLabel,
      }));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showAdd, activeCat, scopedQuestions.length, categories]);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setAddError(null);
    try {
      await apiRequest(`/superadmin/assessments/${activeCode}/questions`, {
        method: "POST",
        body: JSON.stringify({
          ...addForm,
  ...
  const openEdit = (q: Question) => {
    setEditQuestion(q);
    setEditForm({ title: q.title, questionText: q.questionText });
    setEditError(null);
  };
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    setEditError(null);
    try {
      await apiRequest(`/superadmin/questions/${editQuestion._id}`, {
        method: "PATCH",
  ...
  const activeAssessment = assessments.find((a) => a.code === activeCode);
          ...
          <div>
            ...
            >
              ...
              {categories.map(({ cat, label }) => {
                const count = scopedQuestions.filter((q) => q.category === cat).length;
                return (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm bg-white border border-gray-200 text-gray-500 rounded px-1.5 py-0.5">
                        {q.categoryLabel} · Q{q.questionNumber}
                      </span>
                    </div>
                    <p className="text-base text-gray-600 leading-relaxed">{q.questionText}</p>
                  </div>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setAddForm((f) => ({ ...f, questionNumber: Number(e.target.value) }))}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
              ...
              {addError && (
                <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-3 py-2.5 rounded-xl">{addError}</div>
              )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
            ...
            <form onSubmit={handleEdit} className="space-y-4">
              ...
              {editError && (
                <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-3 py-2.5 rounded-xl">{editError}</div>
              )}
